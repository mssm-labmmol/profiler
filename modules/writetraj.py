...
class writer (object):
    ...
    def write_to_fit (self, generation, avg_fit, best_fit):
        ...
        fit_stream.write("{:>5d}{:>18.7e}{:>18.7e}\n".format(generation+1, avg_fit, best_fit))
        fit_stream.close()
    ...
    def write_to_ene (self, generation, population):
        ...
        for k in range(optOpts.nSystems):
            ...
            for j in range(len(refphi)):
                ...
                for i in range(len(population)):
                    ene_stream.write("{:>18.7e}".format(population[i].profiles[k].enerProfile[j]))
                ene_stream.write("\n")
            ene_stream.close()
    ...
    def write_to_pars (self, generation, population):
        ...
        if (dihtype == 'standard'):
            for i in range(6):
                ...
                m_str = "M{}".format(i+1)
                if (i == 0):
                    pars_stream.write("# {:>6}{:>18}{:>5}".format(phi_str,k_str,m_str))
                else:
                    pars_stream.write("{:>8}{:>18}{:>5}".format(phi_str,k_str,m_str))
        ...
        for i in range(len(population)):
            if (dihtype == 'standard'):
                for j in range(6):
                    if (j in optOpts.optTors):
                        ...
                        pars_stream.write("{:>8.2f}{:>18.7f}{:>5d}".format(
                            population[i].phi[k], population[i].k[k], int(population[i].m[k])))
                    else:
                        pars_stream.write("{:>8.2f}{:>18.7f}{:>5d}".format(
                            0.0, 0.0, j+1))                       
            elif (dihtype == 'ryckaert'):
                for j in range(6):
                    if (j in optOpts.optTors):
                        pars_stream.write("{:>18.7e}".format(population[i].k[np.argwhere(optOpts.optTors == j)[0][0]]))
                    else:
            ...
            if (optOpts.nLJ == 0):
                pass
            if (optOpts.nLJ == 1):
                pars_stream.write("{:>18.7e}{:>18.7e}".format(population[i].cs6, population[i].cs12))                
